...
test("should render a read more button with data-testid for long content posts", async ({ browser }) => {
    const context = await browser.newContext();
    const page = await context.newPage();
    await page.goto(baseUrl.client);
    await context.addCookies([{ name: "userId", value: "12", url: `${baseUrl.client}`}]);
...
test("should render a Explore More Posts button with data-testid", async ({ browser }) => {
    const context = await browser.newContext();
    const page = await context.newPage();
    await page.goto(baseUrl.client);
    await context.addCookies([{ name: "userId", value: "12", url: `${baseUrl.client}`}]);
        
    await expect(page.getByTestId('myRecommendedPostsBtn')).toBeVisible();
});
